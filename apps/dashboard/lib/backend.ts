...
import { CLERK_ENABLED, DEV_ORG_ID } from "./auth-config";
...
export interface BackendProvider {
	...
	usage: {
		get(orgId: string, from: string, to: string): Promise<{ data: unknown[]; totals: unknown }>;
	};
	billing: {
...
/** Resolve the orgId from the query string, falling back to DEV_ORG_ID when Clerk is disabled. */
export function resolveOrgId(searchOrgId: string | null): string | null {
	return searchOrgId ?? (!CLERK_ENABLED ? DEV_ORG_ID : null);
}
...
const devBackend: BackendProvider = {
	...
	billing: {
		async get(orgId) {
			return devStore.getBilling(orgId);
		},
...
const prodBackend: BackendProvider = {
	...
	billing: {
		get(orgId) {
			return authedJson(`/v1/orgs/${orgId}/billing`);
		},
...
export function createBackend(): BackendProvider {
	return CLERK_ENABLED ? prodBackend : devBackend;
}
