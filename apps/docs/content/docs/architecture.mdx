...
After flushing deltas to a storage adapter, the gateway can optionally **materialise** them into queryable destination tables. Adapters that implement the `Materialisable` interface produce a denormalised, up-to-date table from the delta log — making the data directly queryable via SQL without replaying deltas.
...
```ts
import { isMaterialisable } from "@lakesync/adapter";
