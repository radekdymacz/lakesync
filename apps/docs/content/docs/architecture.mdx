...
External databases (Neon, Postgres, MySQL) may have data written by services outside LakeSync. The **Source Polling Ingest** system bridges that gap — it periodically queries source tables, detects changes, and pushes them through the gateway as deltas.
...
Two change detection strategies are available: **cursor** (fast, requires a monotonically increasing column like `updated_at`) and **diff** (slower, detects hard deletes via full-table comparison). The poller runs inside `GatewayServer` — no HTTP hop, no auth needed. Detected changes flow through the same conflict resolution and sync rules as client-originated deltas.

See [Source Polling Ingest](/docs/source-polling) for setup, configuration, and strategy comparison.
