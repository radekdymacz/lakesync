...
- **Materialise protocol** — Adapters that implement `Materialisable` can materialise flushed deltas into queryable destination tables with a hybrid column model (synced columns + extensible `props JSONB`).
