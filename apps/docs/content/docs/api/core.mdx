...
Register and create source pollers by connector type.

```ts
function registerPollerFactory(
  type: string,
...
Connector packages (`@lakesync/connector-jira`, `@lakesync/connector-salesforce`) auto-register their factories on import, so `createPoller("jira", config, gateway)` works without manual registration.
