...
class FlushQueueError extends LakeSyncError {}   // code: "FLUSH_QUEUE_ERROR"
```
...
Opt-in capability for adapters that can materialise deltas into queryable destination tables. All three database adapters (PostgresAdapter, MySQLAdapter, BigQueryAdapter) implement this interface via the generic `SqlDialect` + `executeMaterialise()` pattern. See [Materialise Protocol](/docs/architecture#materialise-protocol) for details.
