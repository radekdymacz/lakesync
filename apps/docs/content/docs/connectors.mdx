...
interface ConnectorConfig {
  ...
  type: "postgres" | "mysql" | "bigquery" | "jira" | "salesforce";
  ...
  /** Salesforce CRM config (required when type is "salesforce"). */
  salesforce?: {
    ...
    apiVersion?: string;        // default "v62.0"
    isSandbox?: boolean;        // use test.salesforce.com for auth
    soqlFilter?: string;        // optional WHERE clause fragment
    includeAccounts?: boolean;      // default true
    includeContacts?: boolean;      // default true
    includeOpportunities?: boolean; // default true
    includeLeads?: boolean;         // default true
  };
...
For API connectors (Jira, Salesforce), the `ingest.tables` field is not required — they define their tables internally. Only `intervalMs` is used from the ingest config.
