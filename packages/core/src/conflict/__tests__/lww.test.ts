...
import type { DeltaOp, RowDelta } from "../../delta/types";
...
describe("LWWResolver", () => {
	...
	it("merges non-overlapping columns as a union of both column sets", () => {
		...
		if (result.ok) {
			...
			const titleCol = result.value.columns.find((c) => c.column === "title");
			const doneCol = result.value.columns.find((c) => c.column === "done");
	...
	it("merges columns correctly for INSERT vs UPDATE", () => {
		...
		if (result.ok) {
			...
			const priorityCol = result.value.columns.find((c) => c.column === "priority");
