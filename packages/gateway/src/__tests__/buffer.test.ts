...
import type { HLCTimestamp, RowDelta, DeltaOp } from '@lakesync/core';
...
describe('DeltaBuffer', () => {
	...
	it('shouldFlush triggers at byte threshold', () => {
		...
		expect(
			buffer.shouldFlush({ maxBytes: 100, maxAgeMs: 60_000 }),
		).toBe(false);
		...
		for (let i = 0; i < 5; i++) {
			buffer.append(
				makeDelta({
					...
					deltaId: `delta-${i}`,
					columns: [
						{ column: 'title', value: 'A reasonably long value' },
					],
				}),
			);
		}
		...
		expect(
			buffer.shouldFlush({ maxBytes: 100, maxAgeMs: 999_999 }),
		).toBe(true);
	});
	...
	it('shouldFlush triggers at age threshold', () => {
		...
		expect(
			buffer.shouldFlush({ maxBytes: 999_999, maxAgeMs: 0 }),
		).toBe(true);
	});
